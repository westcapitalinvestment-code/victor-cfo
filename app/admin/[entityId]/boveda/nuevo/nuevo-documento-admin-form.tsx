...
// Igual que nuevo-documento-negocio-form.tsx del dueño, pero con owner_id
// forzado al owner_id EFECTIVO — ver lib/owner-efectivo.ts. El upload de
// archivos (/api/documentos/upload) ya no asume que quien sube ES el
// dueño (fix aplicado en esa ruta el 2 sept 2026).
export default function NuevoDocumentoAdminForm({ entidadId, ownerIdEfectivo }: { entidadId: string; ownerIdEfectivo: string }) {
  ...
  function agregarArchivos(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setArchivos((prev) => [...prev, ...files.map((file) => ({ localId: `${Date.now()}-${Math.random()}`, file, etiqueta: "" }))]);
    e.target.value = "";
  }
  ...
  async function crearDocumento() {
    ...
    for (const archivo of archivos) {
      ...
      if (!res.ok) {
        ...
        setLoading(false);
        setError(data.error ?? "El documento se guardó, pero algún archivo no se pudo subir.");
        return;
      }
    }

    setLoading(false);
    router.push(`/admin/${entidadId}/boveda`);
    router.refresh();
  }
  ...
  return (
    <div className="mx-auto max-w-lg px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={() => router.push(`/admin/${entidadId}/boveda`)} className="text-sm text-muted hover:opacity-80">
          Cancelar
        </button>
      </div>
              ...
              {archivos.map((a) => (
                <li key={a.localId} className="flex items-center gap-2 rounded-lg border border-border p-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <input className="vc-input mt-1" value={a.etiqueta} onChange={(e) => actualizarEtiqueta(a.localId, e.target.value)} />
                  </div>
