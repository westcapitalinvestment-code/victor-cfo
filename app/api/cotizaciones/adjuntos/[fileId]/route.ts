...
// Elimina UN archivo adjunto de una cotización — calcado de
// /api/facturas/adjuntos/[fileId].
export async function DELETE(req: NextRequest, { params }: { params: { fileId: string } }) {
  ...
  const { error: deleteError } = await supabase.from("cotizacion_attachments").delete().eq("id", params.fileId);
