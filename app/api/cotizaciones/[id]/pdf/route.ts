...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (entidad?.logo_r2_key) {
    try {
      ...
      // Caja más grande (pedido de Joel, 25 sept 2026): mismo tamaño que en
      // la factura, para que se vea con más presencia y aire arriba.
  ...
  // Color de marca de la entidad (pedido de Joel, 1 sept 2026) — mismo
  // patrón que /api/facturas: línea del encabezado y totales van en el
  // color que el negocio escogió, con el verde de VICTOR como respaldo.
  ...
  if (entidad?.invoice_footer) {
    // Ancho casi completo (antes se reservaban 180-260pt de más, sobrante de
